...
```bash
devbridge host <tunnelId> -p 8080
```

Host 会自动申请新的 Host 令牌、连接中继服务并开始转发。命令在前台持续运行，按 `Ctrl+C` 停止。
...
```bash
devbridge host <tunnelId> -p 8080 -p 3000
```

对应端口应已经配置在同一条隧道上，而且本机服务应在这些端口正常监听。
