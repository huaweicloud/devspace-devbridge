...
创建成功后会返回隧道 ID、集群、地址和当前到期时间。隧道地址保持为：
...
详情用于确认隧道地址、端口数量、有效期和所属集群。过期或不属于当前工作空间的隧道不会作为有效资源返回。
...
`delete-all` 会影响当前工作空间的所有隧道，应在 CLI 二次确认后执行。删除隧道会同时移除其端口配置，现有 Host 和 Connect 连接也应停止使用该隧道。
