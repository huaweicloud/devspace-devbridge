...
`error_code` 是由数字字符组成的字符串。调用方先根据 HTTP 状态判断错误类别，
再使用错误码处理具体情况：

| HTTP 状态 | 错误码  | 含义                   | 建议处理                       |
| --------- | ------- | ---------------------- | ------------------------------ |
| `400`     | `40000` | 请求参数无效。         | 根据 `error_msg` 修正请求。    |
| `400`     | `11001` | 端口参数无效。         | 检查端口、协议和匿名访问配置。 |
| `401`     | `40100` | 身份认证失败。         | 重新获取或更新调用凭证。       |
| `403`     | `10005` | 无权访问该隧道。       | 检查当前身份的隧道权限。       |
| `403`     | `11004` | 无权访问该端口。       | 检查当前身份的端口权限。       |
| `404`     | `10002` | 隧道不存在。           | 刷新隧道列表，不再使用旧 ID。  |
| `404`     | `11003` | 端口不存在。           | 刷新端口列表。                 |
| `409`     | `10003` | 隧道 ID 冲突。         | 重新提交创建请求。             |
| `409`     | `11002` | 端口已经存在。         | 改为更新端口或选择其他端口。   |
| `410`     | `10004` | 隧道已经过期。         | 创建新隧道。                   |
| `429`     | `10006` | 隧道数量超过配额。     | 删除不再使用的隧道后重试。     |
| `429`     | `42900` | 请求频率超过限制。     | 降低请求频率并进行退避重试。   |
| `500`     | `50000` | 服务暂时无法完成请求。 | 保留请求信息，稍后重试。       |

没有列出的错误码按对应 HTTP 状态处理。不要依赖 `error_msg` 文本进行程序判断，
该字段主要用于日志和问题定位。
